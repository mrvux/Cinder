#include "cinder/dx/resources/DepthStencilArray.h"

using namespace std;

namespace cinder { namespace dx {

DepthStencilArray::Obj::Obj()
{
	mDSV = 0;
	mReadOnlyDSV = 0;
	mResource = 0;
...
DepthStencilArray::DepthStencilArray(DxDevice* device, int w, int h,int d,DXGI_FORMAT format, int samplecount,bool readonly)
	: mObj( shared_ptr<Obj>( new Obj() ) )
{
	mDevice = device;
	mWidth = w;
	mHeight = h;
	Initialize(mDevice,w,h,d,format,samplecount,readonly);
}


DepthStencilArray::~DepthStencilArray(void)
{

}

void DepthStencilArray::Clear(bool depth, bool stencil, float depthvalue,BYTE stencilvalue)
{
...
void DepthStencilArray::Initialize(DxDevice* device, int w, int h,int d,DXGI_FORMAT format, int samplecount,bool readonly)
{
	HRESULT hr;
	...
	ddesc.Format = format;
	ddesc.ViewDimension = samplecount > 1 ? D3D11_DSV_DIMENSION_TEXTURE2DMSARRAY: D3D11_DSV_DIMENSION_TEXTURE2DARRAY;
	...
	srvd.ViewDimension = samplecount > 1 ? D3D11_SRV_DIMENSION_TEXTURE2DMSARRAY : D3D11_SRV_DIMENSION_TEXTURE2DARRAY;
