...
class DepthStencilArray;
typedef std::shared_ptr<DepthStencilArray> DepthStencilArrayRef;

class DepthStencilArray
{
public:
	DepthStencilArray(DxDevice* device, int w, int h,int d,DXGI_FORMAT format = DXGI_FORMAT_D24_UNORM_S8_UINT, int samplecount = 1, bool readonly=false);
	~DepthStencilArray(void);
...
private:
	...
	void Initialize(DxDevice* device, int w, int h,int d, DXGI_FORMAT format = DXGI_FORMAT_D24_UNORM_S8_UINT, int samplecount = 1, bool readonly=false);
